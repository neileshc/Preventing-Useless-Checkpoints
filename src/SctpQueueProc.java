...
public class SctpQueueProc implements Runnable {
	...
	public static Queue<SctpMessage> queueproc = new ConcurrentLinkedQueue();
			...
			if (!queueproc.isEmpty()) {
				...
				if (readytoprocess == true) {
					...
					if (queueproc.peek().msgtype.equals("RECEIVED")) {
						...
						SctpVectorClock.totalmsgssent++;
					} 
					else if (queueproc.peek().msgtype.equals("CheckPoint")) {
						queueproc.poll();

						SctpCheckpoint.take_Checkpoint();
						SctpVectorClock.totalIndependantCKPT++;
						SctpMain.LOG.logger
								.info("\tQUEUE: processing checkpoint");
						readytoprocess = true;

					}

					else if (queueproc.peek().msgtype
							.equals("SENDING_TERMINATION")) {
						queueproc.poll();
						SctpMain.terminate();

						SctpMain.c1.send_terminatation();
						SctpMain.LOG.logger
								.info("\tQUEUE: processing send termination");
						readytoprocess = true;
